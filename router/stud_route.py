...
async def register_student(profile: CreateStudent, db:Session=Depends(reusables_codes.get_db), token:str=Depends(oauth2_scheme)):
    ...
    if user.user_type != "student":
        raise HTTPException(
            ...
            detail=f"ACCESS_DENIED!!! ONLY STUDENTS CAN REGISTER HERE"
        )
    ...
    raise HTTPException(
        status_code=status.HTTP_401_UNAUTHORIZED, 
        detail=f"ALREADY REGISTERED AS >>> {get_student.matric_no.upper()} <<<"
    )
...
async def register_course(input: registerCourse, db:Session=Depends(reusables_codes.get_db), token:str=Depends(oauth2_scheme)) -> Any:
    ...
    if user.user_type != "student":
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED, 
            detail=f"ACCESS_DENIED!!! ONLY STUDENTS CAN REGISTER FOR A COURSE"
        )
    ...
    if not get_course:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED, 
            detail=f"COURSE DOES NOT EXIST YET, TRY LATER."
        )
    ...
    if not get_student:
        raise HTTPException(
            ...
            detail=f"STUDENT DATA NOT FOUND, KINDLY REGISTER."
        )
...
async def course_information(course_code: str, db:Session=Depends(reusables_codes.get_db), token:str=Depends(oauth2_scheme)):
    ...
    if not get_code.first():
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="INVALID INPUT or COURSE NOT YET CREATED"
        )
...
async def my_results(db:Session=Depends(reusables_codes.get_db), token:str=Depends(oauth2_scheme)):
    ...
    if not get_studentid.first():
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, 
            detail="NO RESULTS YET, TRY AGAIN LATER"
        )    
...
async def my_cgpa(db:Session=Depends(reusables_codes.get_db), token:str=Depends(oauth2_scheme)):
    ...
    if not get_userid:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED, 
            detail="USER IS NOT ON STUDENT LIST!!!"
        )       
    ...
    if graded_c_length < registered_c_length:
        raise HTTPException(
            ...
            detail="GRADING IS ONGOING, TRY LATER"
        )    
